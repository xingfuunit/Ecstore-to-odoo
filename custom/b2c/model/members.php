...
class b2c_mdl_members extends dbeav_model{
  ...
  /**
    ...
    function filterData(&$data){
        ...
        unset($data['advance']);
        unset($data['advance_freeze']);
        unset($data['point_freeze']);
        unset($data['point_history']);
        unset($data['point']);
        unset($data['score_rate']);
        unset($data['state']);
    ...
    //精度设置
    public function modifier_advance($row){
        $oCur = app::get('ectools')->model('currency');
        return $oCur->changer_odr($row);
    }
    /**
    * 处理会员中心未读取的数据
    * @author yanglish
    ...
    * @date 2013/1/21
    **/
    public function commnet_list_nums(){
        $msg = kernel::single('b2c_message_msg');
        $row_dis = $msg->getList('*',array('author_id' => $this->app->member_id,'adm_read_status' => 'false','object_type' => 'discuss'));
		$dis_nums = count($row_dis)?count($row_dis):0;
		$row_msg = $msg->getList('*',array('to_id' => $this->app->member_id,'mem_read_status' => 'false','object_type' => 'msg'));
		$msg_nums = count($row_msg)?count($row_msg):0;
		$row_ask = $msg->getList('*',array('author_id' => $this->app->member_id,'mem_read_status' => 'false','object_type' => 'ask'));
		$ask_nums = count($row_ask)?count($row_ask):0;
		$comment_all_nums=array('dis'=>$dis_nums,'ask'=>$ask_nums,'msg'=>$msg_nums);
        return $comment_all_nums;
    }
