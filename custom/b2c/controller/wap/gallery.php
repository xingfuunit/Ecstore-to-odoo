...
//     	print_r($goodsData);exit;
    ...
    private function prepareSeoData($data){
        ...
        return array(
            ...
            'goods_cat'=>empty($this->goods_cat)?'':$this->goods_cat,
...
//        if( isset($tmp_params['tag_id']) && $tmp_params['tag_id'] > 0){
...
//        }
     ...
     * 首页无限滚动接口
     * */
    public function index_scroll_goods(){
        $tmp_params = $this->filter_decode($_GET);
        $params = $tmp_params['filter'];
    ...
    public function get_goods($filter,$page=1,$orderby){
       ...
       // $pageLimit=1;
        ...
        foreach($goodsData as $key=>$goods_row){
            ...
            //$goodsData[$key]['list_title'] = $goods_row['name'].$goods_row['brief'];
            $gids[$key] = $goods_row['goods_id'];
        }
