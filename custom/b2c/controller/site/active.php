...
class b2c_ctl_site_active extends b2c_frontpage{
    ...
    private function meat_active_get_time(){
    	...
    	$db = kernel::database();
    			...
    			$cpns_prefix = 'BSNMY';
    			$coupons_arr = $this->app->model('coupons')->getList("*",array('cpns_prefix'=>$cpns_prefix));
    			$coupons_id3 = $coupons_arr[0]['cpns_id'];
    	...
    	//$sql2 = 'select count(*) as count from sdb_b2c_member_coupon where '.' cpns_id=\''.$coupons_id2.'\' and memc_gen_time>\''.$s_time.'\' and  memc_gen_time<\''.$e_time.'\'';
    	$sql3 = 'select count(*) as count from sdb_b2c_member_coupon where '.' cpns_id=\''.$coupons_id3.'\' and memc_gen_time>\''.$s_time.'\' and  memc_gen_time<\''.$e_time.'\'';
    	 
    	//error_log($sql);
    	$row = $db->select($sql);
    	//$row2 = $db->select($sql2);
    	$row3 = $db->select($sql3);
    	if($row && $row[0]['count']>=50 && $row3 && $row3[0]['count']>=50){
