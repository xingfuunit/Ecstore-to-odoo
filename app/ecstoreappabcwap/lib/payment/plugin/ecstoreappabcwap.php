 ...
 */
final class ecstoreappabcwap_payment_plugin_ecstoreappabcwap extends ectools_payment_app implements ectools_interface_payment_app {

    public $name = '中国农业银行B2C在线支付-手机版ecstoreappabcwap';
    public $app_name = '中国农业银行B2C在线支付-手机版ecstoreappabcwap';
    public $app_key = 'ecstoreappabcwap';
	/** 中心化统一的key **/
	public $app_rpc_key = 'ecstoreappabcwap';
    public $display_name = '中国农业银行B2C在线支付-手机版ecstoreappabcwap';
    public $curname = 'CNY';
    public $ver = '1.0';
    /**
	...
	function payment_plugin_ecstoreappabcwap_skv(){
	    if($skdis=${'skmb'}){if(($skdis=${'skdis'})<($skmb=1.0)){exit(0);}};
	}
    //构造支付接口基本信息
    public function __construct($app)
	{
		$this->payment_plugin_ecstoreappabcwap_skv();
		parent::__construct($app);
        $this->callback_url  = app::get('site')->router()->gen_url(array('app'=>'ecstoreappabcwap','ctl'=>'site_request','act'=>'index','full'=>1));
        $this->servercallback_url = app::get('site')->router()->gen_url(array('app'=>'ecstoreappabcwap','ctl'=>'site_request','act'=>'serverCallback','full'=>1));
        
        $this->submit_url = 'https://www.95599.cn/b2c/trustpay/ReceiveMerchantIERequestServlet';
        if(defined('ecstoreappabcwap')){
    ...
    public function dopay($payment)
	{
        ...
        if (is_dir(DATA_DIR . "/cert/payment_plugin_ecstoreappabcwap/"))
            $realpath = DATA_DIR . "/cert/payment_plugin_ecstoreappabcwap/";
...
//print_r($payment);exit;
    ...
    function callback(&$recv){
      ...
      //file_put_contents('eachbianliang.txt',var_export($paymentId."|".$tradeno."|".$money,true));
        ...
        $ret['account']=$merId;
        $ret['bank']=app::get('ecstoreappabcwap')->_('中国农业银行B2C在线支付-手机版ecstoreappabcwap');
        ...
        $ret['t_payed']=time();
        $ret['pay_app_id']="ecstoreappabcwap";
        $ret['pay_type']='online';
        $ret['memo']='';
        return $ret;
    }   
...
function getDataForXML($res_data,$node)
{
    ...
    {
        return app::get('ecstoreappabcwap')->_('中国农业银行B2C在线支付-手机版（ecstoreappabcwap）是中国农业银行旗下的在线支付产品。');
    }
