...
class b2c_ctl_admin_member extends desktop_controller{
  ...
  function chk_pay_passwd($member_id=null){
  	$member = $this->app->model('members');
  	if($_POST){
  		$userPassport = kernel::single('b2c_user_passport');
  		$this->begin();
  		$member_id = $_POST['member_id'];
  		if ( !$userPassport->check_pay_passport($_POST['newPayPassword'],$_POST['confirmPayPassword'],$msg) ){
  			$this->end(false,$msg);
  		}
  	
  		if ( !$userPassport->reset_passport_pay($member_id,trim($_POST['newPayPassword'])) ){
  	...
  	$this->pagedata['member_id'] = $member_id;
  	$this->display('admin/member/chk_pay_pass.html');
  }
