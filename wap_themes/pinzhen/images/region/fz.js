...
stokeAddress.prototype={
	init:function(config){
	  ...
	  if(config.currentPageLoad.areaCookie!=undefined)
	  {
		 ...
		 var tempDom = $("#stock_province_item a[data-value='"+cookieJsonObj.ProvinceId+"']");
 ...
 // debugger;
  ...
  $("#"+idName).find("a").click(function(){
   ...
   if (config.currentPageLoad.isLoad){
     ...
     {
	   var cookieJsonObj=config.currentPageLoad.areaCookie;
	   var tempDom = $("#"+idName+" a[data-value='"+cookieJsonObj.CityId+"']");
...
reBindStockEvent:function(_this){
	...
	if(_this.requestLevel == 3) {
		...
		$("#store-selector").find(".content, .close").removeAttr("style");
		// $('#stock_area_item, #tab_area_item').css('display','none');
		// $('#tab_city_item').addClass('curr');
		// $('#tab_city_item a').addClass('hover');
