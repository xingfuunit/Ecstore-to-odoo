...
class b2c_ctl_site_product extends b2c_frontpage{
    ...
    private function _get_goods_setting($goods_id){
		...
		//评论数 bySam 20150408
		$sql = "select count(*) as _count from sdb_b2c_member_comments where display = 'true' and for_comment_id = '0' and object_type = 'discuss' and type_id = '{$goods_id}'";
		var_dump($sql);
    ...
    public function index() {
		...
		$sql ="select * from sdb_starbuy_special_goods WHERE product_id = '{$productId}' AND status = 'true'";
    ...
    function goodsDiscuss($gid){
		...
		if(kernel::single('b2c_user_object')->is_login()){	
			$sql = "select order.order_id from sdb_b2c_orders as order,sdb_b2c_order_items as item where order.status = 'finish' AND order.order_id = item.order_id AND item.goods_id = '{$gid}'";
			$discuss = kernel::database()->select($sql);
			if($discuss){
				$this->pagedata['discuss'] = 'is_show';
			}
		}
...
//        if(!empty($_POST['send_address']) && !preg_match('/^(?:[a-z\d]+[_\-\+\.]?)*[a-z\d]+@(?:([a-z\d]+\-?)*[a-z\d]+\.)+([a-z]{2,})+$/i', $_POST['send_address'])){
//            $this->splash('failed', $back_url, app::get('b2c')->_('邮箱格式错误'),true);
//            $error = 1;
//        }else{
//            $_POST['email'] = $_POST['send_address'];
//        }
//        if(empty($_POST['email']) && ){
//            $this->splash('failed', $back_url, app::get('b2c')->_('手机格式错误'),true);
//        }
        if(preg_match('/^(?:[a-z\d]+[_\-\+\.]?)*[a-z\d]+@(?:([a-z\d]+\-?)*[a-z\d]+\.)+([a-z]{2,})+$/i', $_POST['send_address'])){
			$_POST['email'] = $_POST['send_address'];
        }elseif(strlen($_POST['send_address']) == 11 && preg_match('/1[345789][123569]{1}\d{8}/',$_POST['send_address'])){
			$_POST['cellphone'] = $_POST['send_address'];
		}else{
			$this->splash('failed', $back_url, app::get('b2c')->_('所输入地址的格式错误'),true);
		}
        $objGoods = $this->app->Model('goods');
