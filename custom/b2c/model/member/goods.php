...
###删除收藏商品
    ...
    function parent_count($filter=null){
	 ...
	 function goods_favorite_count($goods_id = 0){
	 
	 	$count = $this->db->selectrow("SELECT COUNT(member_goods.`gnotify_id`) AS num
									FROM ".$this->table_name(1)." AS member_goods WHERE member_goods.`goods_id`=".$goods_id." AND member_goods.`type`='fav' ");
	 	return $count['num'];
	 }
...
####根据会员ID获得该会员收藏的商品
    ...
    function get_favorite($member_id,$member_lv_id,$page=1,$num=10){
        ...
        if(is_array($agid)&&$agid){
            ...
            $oGoodsLv = $this->app->model('goods_lv_price');
            $oGood = $this->app->model('goods');
            $aProduct = $oGood->getList('cat_id,udfimg,thumbnail_pic,image_default_id,goods_id,price,name,type_id,nostore_sell,marketable',array('goods_id' => $agid));
            ...
            if($aProduct){
                ...
                foreach ($aProduct as &$val) {
                    ...
                    $val['goods_favorite_count'] = $this->goods_favorite_count($val['goods_id']); //总收藏数
            ...
            // 找出父id
            $cats_path = $this->app->model('goods_cat')->getList("cat_path,cat_name, cat_id",array('cat_id'=>$cat_id));
            foreach ($params['data'] as $key => $value) {
              foreach ($cats_path as $k => $v) {
                if ($value['cat_id'] == $v['cat_id']) {
                  $tmpid = explode(',', $v['cat_path']);
                  $params['data'][$key]['parent_cat_id'] = $tmpid[1];
                }
              }
            }
