...
{    
    ...
    public function pay_finish(&$sdf, $status='succ',&$msg='')
    {
                ...
                case 'payments':
                    switch ($objOrderbills['pay_object'])
                    {
                        ...
                        case 'recharge':
                            ...
                            file_put_contents('f.log', serialize($sdf)."\r\n".serialize($objOrderbills)."\r\n",FILE_APPEND);
                            // 增加经验值
        ...
        //充值卡充值
        $orders_select = $this->app->model('orders');
        $arrOrders = $orders_select->dump($objOrderbills['rel_id'],'*');
        if($arrOrders['czkcz_is']==1 && $arrOrders['pay_status']==1){
            $orders_czkcz = $this->app->model('orders');
            $save_data['czkcz_is']=2;
            $orders_czkcz->update($save_data, array('order_id' => $arrOrders['order_id']));
            $member_id = $arrOrders['member_id'];
            $msg = '充值卡充值';
            $objAdvances = $this->app->model("member_advance");
            $objAdvances->add($member_id, $arrOrders['total_amount'], app::get('b2c')->_('充值卡充值'), $msg);
        }
