...
=======
      //  $arrMember = kernel::single('b2c_user_object')->get_current_member(); //member_id,uname
      $arrMember['member_id']=$_POST['member_id'];
...
//            }
            ...
            $mobile=$_POST['mobile']; 
            $addr=app::get('ome')->model('branch')->getList('*',array('branch_id'=>$branch_id));
            $ship_area_name = app::get('ectools')->model('regions')->change_regions_data($addr[0]['area']);
            
            $_POST['delivery']['addr_id'] = $addr[0]['branch_id'];
            $_POST['delivery']['ship_area'] = $addr[0]['area'];
            $_POST['delivery']['ship_addr'] = $ship_area_name.$addr[0]['addr'];
            $_POST['delivery']['ship_zip'] = $addr[0]['zip'];
           // $_POST['delivery']['ship_name'] = $addr[0]['name'];
            $_POST['delivery']['ship_name'] =  $addr[0]['name'];
            $_POST['delivery']['ship_mobile'] =$mobile;
            $_POST['delivery']['ship_tel'] = $addr[0]['phone'];
            //if( $this->app->getConf('site.checkout.receivermore.open') == 'true'){
                $_POST['delivery']['day'] = $addr[0]['day']?$addr[0]['day']:app::get('b2c')->_('任意时间');
                $_POST['delivery']['time'] = $addr[0]['time']?$addr[0]['time']:app::get('b2c')->_('任意时间');
//            }else{
//                $_POST['delivery']['day'] = app::get('b2c')->_('任意时间');
//                $_POST['delivery']['time'] = app::get('b2c')->_('任意时间段');
//            }
...
=======
        ...
        if (!$_POST['delivery']['ship_area'] ||  !$_POST['delivery']['ship_addr'] || !$_POST['delivery']['ship_name'] ||  (!$_POST['delivery']['ship_mobile'] && !$_POST['delivery']['ship_tel']) || !$_POST['payment']['pay_app_id'])
        {
//            if (!$_POST['delivery']['ship_area'] )
//            {
//                $msg .= app::get('b2c')->_("收货地区不能为空！")."<br />";
...
=======
      //  $_POST['member_id'] = $arrMember['member_id'] ? $arrMember['member_id'] : 0;
        $_POST['member_id']=$_POST['member_id'];
        $order_data = array();
        $obj_order_create = kernel::single("b2c_order_create");
...
=======
        ...
        $order_data['staff_id']=$_POST['staff_id'];
        $staff=$this->app->model('local_staff')->dump(array('staff_id'=>$order_data['staff_id']));
        $order_data['staff_name']=$staff['staff_name'];
       
        $result = $obj_order_create->save($order_data, $msg);
        if ($result)
        {
            
             //如果手机号是已注册会员，则给予积分和经验值
                $buymember=  app::get('pam')->model('members')->getList('*',array('login_account'=>$mobile));
              
                if($buymember){
                    
                    $experience=  floor($order_data['total_amount']);//订单金额向下取整
                    //增加经验值
                    if($experience>0){
                        if(!app::get('b2c')->model('members')->change_exp($buymember[0]['member_id'],$experience,$msg)){
                            kernel::single('base_rpc_service')->send_user_error('order_error', $msg);
                        }                       
                    }
                   
                    $point=0;
                    $get_policy=app::get('b2c')->getConf('site.get_policy.method');
                    
                    //获取积分计算方式 1不适用积分，2按订单总价格计算，3按单独为商品设置积分算
                    if($get_policy=='2'){                      
                        $rate=app::get('b2c')->getConf('site.get_rate.method'); //获取积分换算比例
                        $point=floor($order_data['total_amount']* $rate);//计算所得积分
                       
                        if($point>0){
                           if(!app::get('b2c')->model('member_point')->change_point($buymember[0]['member_id'],$point,$msg,'order_pay_get',2,$order_data['order_id'],$buymember[0]['member_id'])){
                               kernel::single('base_rpc_service')->send_user_error('order_error', $msg);
                           }
                        }
...
//     		return true;
 ...
 //-----erp api ------
    ...
    public function delivery_sign(){
    	...
    	$order_ids_str = $_POST['order_ids_str'];
    	
    	$sql = "UPDATE `sdb_b2c_orders` SET  `delivery_sign_status` =  '1' where order_id in (".$order_ids_str.")";
    	$order_up = kernel::database()->exec($sql);
    	if($order_up)return "succ";
    	kernel::single('base_rpc_service')->send_user_error('error', "更新失败");
    }
