...
class base_db_connect extends base_db_abstract implements base_interface_db
{

    protected $_rw_lnk = null;
    protected $_ro_lnk = null;
    protected $host = ERP_DB_HOST;
    protected $user = ERP_DB_USER;
    protected $passwd = ERP_DB_PASSWORD;
    protected $dbname = ERP_DB_NAME;
    protected $timeout = ERP_DB_TIMEOUT;
    ...
    public function test_mysqli_timeout(){
    	//创建对象
    	$mysqli = mysqli_init();
    	//设置超时选项
    	$mysqli->options(MYSQLI_OPT_CONNECT_TIMEOUT, $this->timeout);
    	//连接
    	$mysqli->real_connect($this->host, $this->user, $this->passwd,$this->dbname);
    	//如果超时或者其他连接失败打印错误信息
    ...
    protected function _rw_conn(){
        $this->_rw_lnk = $this->_connect($this->host, $this->user, $this->passwd,$this->dbname);
        return $this->_rw_lnk;
    }
