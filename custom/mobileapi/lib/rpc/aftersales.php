...
class mobileapi_rpc_aftersales extends mobileapi_rpc_member
{
    ...
    public function afterlist($params = array())
    {
        ...
        if (!isset($obj_return_policy) || !is_object($obj_return_policy))
        {
            $this->app->rpcService->send_user_error('4003', '售后服务应用不存在！');
        }

        if (!$obj_return_policy->get_conf_data($arr_settings))
        {
            $this->app->rpcService->send_user_error('4003', '售后服务信息没有取到！');
        }
    ...
    public function add($params = array())
    {
        ...
        $data['order'] = $objOrder->dump($order_id, '*', $subsdf);

        // 校验订单的会员有效性.
        $is_verified = ($this->verify_member($data['order']['member_id'])) ? $this->verify_member($data['order']['member_id']) : false;

        // 校验订单的有效性.
        if ($_COOKIE['ST_ShopEx-Order-Buy'] != md5($this->app->getConf('certificate.token').$order_id) && !$is_verified)
        {
            $this->app->rpcService->send_user_error('4003', '订单无效！');
        }

        //$data['orderlogs'] = $objOrder->getOrderLogList($order_id);
        ...
        $oGoods = app::get('b2c')->model('goods');
        $imageDefault = app::get('image')->getConf('image.set');
        foreach ($data['order']['order_objects'] as $k=>$arrOdr_object)
        {
            $index = 0;
            $index_adj = 0;
            $index_gift = 0;
            ...
            if ($arrOdr_object['obj_type'] == 'goods')
            {
                foreach($arrOdr_object['order_items'] as $key => $item)
                {
                    ...
                    if ($tmp_array = $arr_service_goods_type_obj[$item['item_type']]->get_aftersales_order_info($item)){
                        ...
                        $order_items[$product_id]['thumbnail_pic_scr'] = base_storager::image_path($order_items[$product_id]['thumbnail_pic'], 's' );
                    }
                }
            }
            else
    ...
    public function return_save()
    {
        ...
        if (!isset($obj_return_policy) || !is_object($obj_return_policy))
        {
            $this->app->rpcService->send_user_error('error', '售后服务应用不存在！');

        }

        if (!$obj_return_policy->get_conf_data($arr_settings))
        {
            $this->app->rpcService->send_user_error('error', '售后服务信息没有取到！');
        }

        if (!$_POST['product_bn'])
        {
            $this->app->rpcService->send_user_error('error', '您没有选择商品，请先选择商品！');
        }

        if (!$_POST['title'])
        {
            $this->app->rpcService->send_user_error('error', '请填写退货理由');
        }

        $upload_file = "";
        if ( $_FILES['file']['size'] > 314572800 )
        {
            $this->app->rpcService->send_user_error('error', '上传文件不能超过300M！');
        }
        ...
        if ( $_FILES['file']['name'] != "" )
        {
            ...
            {
                $text = implode(",", $type);
                $this->app->rpcService->send_user_error('error',app::get('aftersales')->_("您只能上传以下类型文件: ") . $text . "<br>");
            }
        ...
        if(!$_POST['agree']){
            $this->app->rpcService->send_user_error('error',app::get('aftersales')->_("请先查看售后服务须知并且同意"));
        }
        ...
        {
            $this->app->rpcService->send_user_error('error', $msg);
        }
    }
