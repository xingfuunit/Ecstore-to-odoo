...
class b2c_ctl_site_gallery extends b2c_frontpage{
	...
	private function get_marketable($goodsData) {
		$goods_ids = array();
		foreach ($goodsData as $key=>$value) {
			$goods_ids[] = $value['goods_id'];
		}
        $productModel = $this->app->model('products');
        $products =  $productModel->getList('*',array('goods_id'=>$goods_ids));
        
        foreach ($goodsData as $key=>$value) {
        	
        	foreach ($products as $ky=>$val) {
        		if ($value['goods_id'] == $val['goods_id']) {
        			foreach ($value['spec_desc'] as $k1=>$v1) {
        				foreach ($v1 as $k2=>$v2) {
        					if ($v2['spec_value_id'] == current($val['spec_desc']['spec_value_id'])) {
        						$goodsData[$key]['spec_desc'][$k1][$k2]['marketable'] = $val['marketable'];
        					}
        				}
        			}
        		}
    ...
    //把多维数据转变为一维数据 by hnqss.cn
    function arrayChange($arrays){
        static $aresult;
        ...
        /*
         * ajax方式 根据goods_id、spec_value_id、spec_id取product_id  by hnqss.cn
         * */
    public function ajax_get_product_id(){
            $goods_id=$_POST['goods_id'];
            $spedata=$_POST['spedata'];
            $specimage=$_POST['speimage'];
            $simgsrc="hnqss.cn";  // 此处省去前台判断是否需要分隔
            if(empty($goods_id) || empty($spedata) || !strstr($spedata,'|')){return false;}
            //处理image src
            if($specimage){
                if(strstr($specimage,",")){
                    $spimsrcs=explode(",",$specimage);
                    $simgsrc= base_storager::image_path($spimsrcs[0]);
                }
                else{
                    $simgsrc= base_storager::image_path($specimage);
                }
            }
            //得到product_id
            $o_specs=app::get('b2c')->model('goods_spec_index');
            if(strstr($spedata,",")){
                $spelist=explode(",",$spedata);
                foreach($spelist as $key=>$val){
                    if(strstr($val,"|")){
                        $speset=explode("|",$val);
                       $result=$this->arrayChange($o_specs->getList('product_id',array('spec_id'=>$speset[0],'spec_value_id'=>$speset[1],'goods_id'=>$goods_id)));
                     }
                 }
                ...
                $speset=explode("|",$spedata);
               $result=$o_specs->getList('product_id',array('spec_id'=>$speset[0],'spec_value_id'=>$speset[1],'goods_id'=>$goods_id));
                $product_id= end($result[0]);
            }
            //得到价格 市场价 销售价
            if($product_id){
                $o_product=app::get('b2c')->model('products');
                $prolist=$o_product->getList('price,mktprice',array('product_id'=>$product_id,'goods_id'=>$goods_id));
                foreach($prolist as $key=>$val){
                    $spprice=number_format($val['price'],2);
                    $spmktprice=number_format($val['mktprice'],2);
                }
             }
            echo $product_id."-".$simgsrc."-".$spprice."-".$spmktprice;
    }
    /*
     * 返回搜索条件
     *
