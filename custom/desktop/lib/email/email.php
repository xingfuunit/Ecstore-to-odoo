  ...
  function send($to, $subject, $body, $config){
        ...
        switch($config['sendway'])
        {
            ...
            case "smtp":
                ...
                $result = $this->SmtpSsl($to,$subject, $body,$config);//换成支持ssl
                break;
            default:
...
//            trigger_error("recipients_failed". $to);
	...
	//调新的支持ssl的类
	function SmtpSsl($to,$subject, $body,$config) {
    	//调支持ssl的smtp类
    	$app = app::get('desktop');
    	$isssl = $app->getConf('email.config.isssl') ? true : false;
    	$ssl = kernel::single('desktop_email_ssl');
    	$ssl->setServer($config['smtpserver'], $config['usermail'], $config['smtppasswd'], $config['smtpport'], $isssl);
    	$smtp_from = $this->Sender;
    	
		$ssl->setFrom($smtp_from); //设置发件人
		$ssl->setReceiver($to); //设置收件人，多个收件人，调用多次
	//	echo $subject.'<br />';
	//	var_dump($body);
	//	exit;
		$ssl->setMail($subject, base64_decode($body)); //设置邮件主题、内容
		$send = $ssl->sendMail(); //发送
		return $send;
	}
