...
class ectools_ctl_payment_cfgs extends desktop_controller{
    ...
    public function _views(){
        ...
        $wx_filter = array('platform'=>'iswx', 'is_frontend' => false);
        $com_filter = array('platform'=>'iscommon', 'is_frontend' => false);

        $all_num = count($o->getList('*',$all_filter));
        $pc_num = count($o->getList('*',$pc_filter));
        $mobile_num = count($o->getList('*',$mobile_filter));
        $wx_num = count($o->getList('*',$wx_filter));
        $com_num = count($o->getList('*',$com_filter));
        ...
        $show_menu = array(
        	...
        	4=>array('label'=>app::get('ectools')->_('微信版'),'optional'=>false,'addon'=>$wx_num, 'filter'=>$wx_filter),
        	5=>array('label'=>app::get('ectools')->_('通用版'),'optional'=>false,'addon'=>$com_num, 'filter'=>$com_filter)
        );
        return $show_menu;
    }
