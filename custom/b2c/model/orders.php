...
class b2c_mdl_orders extends dbeav_model{
    ...
    public function fetchByMember($member_id, $nPage=1, $order_status=array(),$limit=10)
    {
        ...
        else
        {
            ...
            if (isset($order_status['createtime_to']))
                $filter['createtime_to'] = $order_status['createtime_to'];
            if (isset($order_status['createtime_from']))
                $filter['createtime_from'] = $order_status['createtime_from'];
        }
    ...
    private function _filter_sql($filter) {
        ...
        if (isset($filter['createtime_from'])) {
            $where .= ' AND createtime >='.$filter['createtime_from'];
        }
        if (isset($filter['createtime_to'])) {
            $where .= ' AND createtime <='.$filter['createtime_to'];
        }
        return $where;
    }
