...
class JSSDK {
  ...
  private function getJsApiTicket($bind_id) {
      ...
      $accessToken = $kernel::single('weixin_wechat')->get_basic_accesstoken();
      ...
      if ($jsapi_ticket) {
      	if( !base_kvstore::instance('weixin')->store('basic_jsapi_ticket_'.$bind_id, $jsapi_ticket, time() + 7000) ){ // 微信jsapi_ticket的有效期,单位为秒
