...
class mobileapi_rpc_cart  extends mobileapi_frontpage {
    ...
    public function get_stores_list($params, $rpcService) {
        ...
        */
    	...
    	if(isset($params['area_id']) && $params['area_id'] > 0){
    		$region = kernel::single('ectools_mdl_regions')->dump($params['area_id'], 'region_path');
    		$_filter['region_path|head'] = $region['region_path'];
    	}
    	
        $list = app::get('ome')->model('branch')->getList('branch_id, name, address', $_filter);
        return $list;
    }
     ...
     * 凑单
     * */
    public function fororder($params) {
        ...
        $data['fororder_tab'] = $new_fororder_tab;

        if(isset($params['tab_name'])){
        ...
        foreach($list as $goods_item){
            $goods_item['default_img_url'] = base_storager::image_path($goods_item['image_default_id'], 'm' );
            $goodsData[] = $goods_item;
        }
