...
class b2c_user_object{
    ...
    public function get_member_name_weixin($member_id=null){
        $pam_tag_model = app::get('pam')->model('bind_tag');
        $row_member = $pam_tag_model->getList('*',array('member_id'=>$member_id));
        return $row_member[0]['tag_name'];
    }
    /**
     * 获取当前会员用户名/或指定用户的用户名
     */
