...
function theme_widget_qf_goodscatnav(&$setting,&$render){
	...
	foreach($setting['gcatlinks'] as $key=>$val){
			if($val=='%THEME%/images/' || $val==''){
				$_returnData['gcatlinks'][]='';
				}
			else{
				$_returnData['gcatlinks'][]=$val;
				}
			
		}
	$_returnData['imglinks']=$setting['imglinks'];;
    return $_returnData;
