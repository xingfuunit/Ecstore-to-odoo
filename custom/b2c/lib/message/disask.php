...
class b2c_message_disask extends b2c_message_comment{
    ...
    function getGoodsCommentAndReplyListEx($goods_id,$last_comment_id,$limit){
        if(!$goods_id) return;

        $item = 'discuss';
        $setting = $this->disask_setting($item);
        if($setting['switch'] == 'on'){
            $commentList = $this->getGoodsCommentListEx($goods_id,$last_comment_id,$limit);

            $aComment['list'][$item] = $commentList['data'];

            $aComment['last_comment_id'] = $commentList['last_comment_id'];
     ...
     * @author francis
     */
    function getGoodsCommentListEx($goods_id,$last_comment_id,$limit){
        $item = 'discuss';
        ...
        if($last_comment_id != 0){
            $filter['comment_id|lthan'] = $last_comment_id;
        }
        ...
        $aData = $this->getList('*',$filter,0,$list_listnum);
        $data = array();
        ...
        $result['data'] = $data;
        $result['last_comment_id'] = $last_comment_id;
        return $result;
    }
