...
class groupactivity_ctl_admin_purchase extends desktop_controller
{
    ...
    public function edit()
    {
        ...
        $arr_goods = app::get('b2c')->model('products')->getList( '*',array('product_id'=>$arr['pid']) );
    ...
    {
        $data = $_POST['data'];
        $arr = app::get('b2c')->model('products')->getList( '*',array('product_id'=>$data[0]) );
        reset( $arr );
        $arr = current($arr);
        echo json_encode( array('name'=>$arr['name'],'price'=>$arr['price'],'store'=>(INT)$arr['store'],'product_id'=>$arr['product_id']) );
    }
    ...
    public function toAdd()
    {
        ...
        $aGoods = app::get('b2c')->model('products')->getList('store',array('product_id'=>$data['pid']) );
        $aGoods = $aGoods[0];
        $arr = $o->getList( '*',array('pid'=>$data['pid'],'end_time|than'=>time()) );
        if( $arr && !$data['act_id'] ) {
        ...
        if( isset( $aGoods['store'] )){//无库存状态应该判断可以购买
