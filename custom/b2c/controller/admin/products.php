...
/**
 * 
 * hack by Jason 此文件是从源码版ecstore2.3.10的app目录下拷贝过来的,添加了获取货品的条形码的字段barcode.
 *
 */
...
class b2c_ctl_admin_products extends desktop_controller{
    ...
    public function getProducts($gid=0){
        ...
        $productData = $productMode->getList('product_id,barcode,bn,price,cost,mktprice,store,freez,store_place,weight,marketable,spec_desc,is_default',array('goods_id'=>$gid));
        foreach((array)$productData as $row){          //hack by Jason 在上面的getList方法,添加了返回的字段barcode
 ...
 /*--------------------------以下为保存货品函数--------------------------------------------*/
    ...
    private function _pre_process_products($products,&$spec){
        ...
        foreach($products as $uid=>$row)
        {
            ...
            $tmpProducts[$uid]['barcode'] = $row['barcode'];
