...
$log_text[] =
    ...
    $db->commit($transaction_status);
            ...
            if (!$sdf['logi_no'] && $sdf['status'] == 'succ')
            {
                ...
                //发信息，如果是仓库门店自提不发信息 @iegss
        		$branch_order = $order->dump($aUpdate['order_id'],"branch_id,order_id");
        		if(!$branch_order['branch_id'])$order->fireEvent('shipping', $aUpdate, $sdf_order['member_id']);
