...
class b2c_ctl_site_member extends b2c_frontpage{
    ...
    function favorite($nPage=1){
        ...
        foreach($aProduct as $k=>$v){
            ...
            if($v['marketable'] == 'false'){
                kernel::single('b2c_member_fav')->del_fav($this->app->member_id,'goods',$v['goods_id']);
                unset($aProduct[$k]);
            }
        }
        $this->pagedata['browser'] = $this->get_browser();
    ...
    */
    ...
    public function nodiscuss($nPage=1){
        ...
        //获取会员已发货的商品日志
        $sell_logs = $this->app->model('sell_logs')->getList('order_id,product_id,goods_id,createtime',array('member_id'=>$this->app->member_id));
        //获取会员已评论的商品
