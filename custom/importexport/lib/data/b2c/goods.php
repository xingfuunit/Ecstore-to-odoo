...
class importexport_data_b2c_goods { 
    ...
    public function dataToSdf($contents, &$msg){
        ...
        $save_data = array(
            ...
            'props'  => $this->_import_props($goods, $type_id),
    ...
    //导入扩展属性
    public function _import_props($goods, $type_id=0){
        $props = array();
        foreach( $goods as $col=>$value){
            ...
            if( substr($col,0,5) == 'props' ){
                ...
                if( $goods_p <= 20 ) {
                    //用属性过滤条件，区分重名的属性 begin
                    $propslist = $propsids = array();
                    if($type_id>0){
                            $propslist = app::get('b2c')->model('goods_type_props')->getList('props_id',array('type_id'=>$type_id));
                            foreach ($propslist as $pl){
                                    $propsids[] = $pl['props_id'];
                            }
                    }
                    $props_filter = array('name'=>$value);
                    if(!empty($propsids)){
                            $props_filter = array('name'=>$value,'props_id|in'=>$propsids);
                    }
                     //用属性过滤条件，区分重名的属性 end
                    $propsData = app::get('b2c')->model('goods_type_props_value')->getList('props_value_id',$props_filter); 
